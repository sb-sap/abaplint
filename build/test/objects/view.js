...
describe("View, parse XML", () => {
    it("test", () => {
        const xml = "<?xml version=\"1.0\" encoding=\"utf-8\"?>\n" +
            ...
            " </asx:abap>\n" +
            "</abapGit>";
        const reg = new registry_1.Registry().addFile(new memory_file_1.MemoryFile("zag_unit_testv.view.xml", xml)).parse();
        const tabl = reg.getObjects()[0];
        chai_1.expect(tabl.getName()).to.equal("ZAG_UNIT_TESTV");
        const fields = tabl.getFields();
        chai_1.expect(fields.length).to.equal(4);
        chai_1.expect(fields).to.contain("PGMID");
        chai_1.expect(fields).to.contain("OBJECT");
        chai_1.expect(fields).to.contain("OBJ_NAME");
        chai_1.expect(fields).to.contain("KORRNUM");
    });
});
