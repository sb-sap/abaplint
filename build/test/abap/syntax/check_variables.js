...
const check_variables_1 = require("../../../src/abap/syntax/check_variables");
function run(reg) {
    let ret = [];
    for (const obj of reg.getABAPObjects()) {
        ret = ret.concat(new check_variables_1.CheckVariablesLogic(reg, obj).findIssues(false));
    }
    return ret;
}
function runMulti(objects) {
    const reg = new registry_1.Registry();
    for (const obj of objects) {
        const file = new files_1.MemoryFile(obj.filename, obj.contents);
        reg.addFile(file).parse();
    }
    return run(reg);
}
function runClass(abap) {
    const file = new files_1.MemoryFile("zcl_foobar.clas.abap", abap);
    const reg = new registry_1.Registry().addFile(file).parse();
    return run(reg);
}
function runProgram(abap) {
    const file = new files_1.MemoryFile("zfoobar.prog.abap", abap);
    const reg = new registry_1.Registry().addFile(file).parse();
    return run(reg);
}
describe("Check Variables", () => {
    it("program, variable foobar not found", () => {
        ...
        chai_1.expect(issues[0].getMessage()).to.equal("\"foobar\" not found");
    });
    ...
    it("program, constant, begin", () => {
        const abap = "CONSTANTS: BEGIN OF c_mode,\n" +
    ...
    it("program, different scope", () => {
        ...
        const issues = runProgram(abap);
        chai_1.expect(issues.length).to.equals(1);
        chai_1.expect(issues[0].getMessage()).to.equal("\"moo\" not found");
    });
    ...
    */
    ...
    it("class, method not found, must push scope", () => {
        const abap = "CLASS zcl_foobar DEFINITION PUBLIC FINAL CREATE PUBLIC.\n" +
            ...
            "ENDCLASS.";
        const issues = runClass(abap);
        chai_1.expect(issues.length).to.equals(2);
    });
    it("class, attribute", () => {
        const abap = "CLASS zcl_foobar DEFINITION PUBLIC FINAL CREATE PUBLIC.\n" +
            "  PUBLIC SECTION.\n" +
    ...
    it("function module definition not found", () => {
        ...
        chai_1.expect(issues.length).to.equals(1);
        chai_1.expect(issues[0].getMessage()).to.contain("not found");
    });
    it("function module", () => {
    ...
    it("redefined method with parameter, 2 steps up", () => {
        const clas = "CLASS zcl_foobar DEFINITION PUBLIC INHERITING FROM zcl_super1 FINAL CREATE PUBLIC.\n" +
            "  PUBLIC SECTION.\n" +
            "    METHODS method1 REDEFINITION.\n" +
            "ENDCLASS.\n" +
            "CLASS zcl_foobar IMPLEMENTATION.\n" +
            ...
            "  ENDMETHOD.\n" +
            "ENDCLASS.\n";
        const sup1 = "CLASS zcl_super1 DEFINITION PUBLIC INHERITING FROM zcl_super2 CREATE PUBLIC.\n" +
            "ENDCLASS.\n" +
            "CLASS ZCL_SUPER1 IMPLEMENTATION.\n" +
            "ENDCLASS.";
        const sup2 = "CLASS zcl_super2 DEFINITION PUBLIC CREATE PUBLIC.\n" +
            "  PUBLIC SECTION.\n" +
            "    METHODS method1 IMPORTING parameter TYPE c.\n" +
            "ENDCLASS.\n" +
