...
const tokens_1 = require("../../src/abap/tokens");
const str = Combi.str;
const per = Combi.per;
const seq = Combi.seq;
const alt = Combi.alt;
...
function tokenize(s) {
    return lexer_1.Lexer.run(new files_1.MemoryFile("foo.abap", s));
}
const tests = [
    ...
    { n: "tok1", c: tok(tokens_1.Identifier), t: tokenize("FOO"), e: true },
    { n: "tok2", c: seq(str("A"), tok(tokens_1.WPlusW), str("B")), t: tokenize("A + B"), e: true },
    { n: "tok3", c: seq(str("A"), tok(tokens_1.Plus), str("B")), t: tokenize("A+B"), e: true },
...
describe("combi matching -", () => {
    tests.forEach((test) => {
        it(test.n, () => {
            ...
            chai_1.expect(match).to.equals(test.e);
            if (test["len"] !== undefined) {
                chai_1.expect(test["len"]).to.equals(result.length);
            }
        });
    });
});
