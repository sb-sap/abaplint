...
describe("Test expression matchers", () => {
    tests.forEach((test) => {
        ...
        it("\"" + test.c + "\" should " + not + "match " + test.r.getName(), () => {
            const tokens = _utils_1.getTokens(test.c);
            const match = Combi.Combi.run(test.r.getRunnable(), tokens, config_1.Config.getDefault().getVersion());
            chai_1.expect(match !== undefined).to.equals(test.e);
        });
    });
});
